...
@pytest.mark.asyncio
async def test_already_recorded_completion_retries_capture_without_rewriting_outcome():
    reconciler = DurableOutcomeReconciler.__new__(DurableOutcomeReconciler)
...
@pytest.mark.asyncio
async def test_richer_outcome_is_never_downgraded_to_track_completion():
    reconciler = DurableOutcomeReconciler.__new__(DurableOutcomeReconciler)
    reconciler._launch_history = SimpleNamespace(record_outcome=AsyncMock())
