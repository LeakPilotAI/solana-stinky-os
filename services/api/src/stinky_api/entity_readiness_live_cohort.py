...
from stinky_api.entity_readiness_component_maturity import readiness_component_maturity
...
async def live_entity_readiness_cohort_validation(
    ...
    *,
    ...
    cohort_entity_ids = [
        ...
        for item in (prospective_replay.get("entities") or [])
        if isinstance(item, dict) and item.get("entity_id")
    ]
    cutoff = as_of if isinstance(as_of, datetime) else None
    summary["component_maturity"] = await readiness_component_maturity(
        session,
        entity_ids=cohort_entity_ids,
        not_before=PROSPECTIVE_CAPTURE_EPOCH,
        as_of=cutoff,
    )
    if include_entities:
