...
def describe_developer_correlation_change(previous: dict[str, Any] | None, current: dict[str, Any]) -> dict[str, Any]:
    ...
    prev = _canonical_payload(previous)
    ...
    for field, added_kind, removed_kind in specs:
        before = set(prev.get(field) or [])
        after = set(cur.get(field) or [])
        added = sorted(after - before)
        removed = sorted(before - after)
        if added:
            changes.append({"kind": added_kind, "field": field, "added": added, "removed": []})
        if removed:
            changes.append({"kind": removed_kind, "field": field, "added": [], "removed": removed})
    before_missing = set(prev.get("missing") or [])
    after_missing = set(cur.get("missing") or [])
...
async def persist_developer_correlation_snapshot(
...
async def developer_correlation_audit_history(
    session: AsyncSession,
    entity_id: str,
    *,
    limit: int = 20,
    ...
    for row in chronological:
        ...
        change = describe_developer_correlation_change(previous, evidence)
        change["observed_at"] = record["observed_at"]
        change["evidence_hash"] = record["evidence_hash"]
...
async def developer_correlation_change_feed(
    session: AsyncSession,
    *,
    limit: int = 50,
    as_of: datetime | None = None,
