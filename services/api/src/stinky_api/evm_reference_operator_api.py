"""Operator-facing, read-only API for durable reference DEX observation.

Transport validation lives here; observation semantics remain in executions #248-#250.
RPC URLs are never accepted from HTTP payloads. Observers must be preconfigured and
injected into ``app.state.reference_evm_observers`` by chain key.
"""
...
from stinky_api.evm_reference_observation_operator import (
    DurableReferenceDexObservationResult,
    invoke_reference_dex_observation,
)
...
router = APIRouter(prefix="/v1/operator/evm-reference", tags=["operator", "evm-reference"])
...
def build_operator_request(
    ...
    """Convert validated transport primitives into existing immutable runtime types."""
    ...
    router_address = _canonical_address(payload.chain, payload.router_address, "router_address")
    if token0 == token1:
...
def get_reference_observers(request: Request) -> tuple[EvmReadOnlyRpc, ...]:
    """Return deployment-injected observers; HTTP callers cannot choose RPC URLs."""
    observers_by_chain = getattr(request.app.state, "reference_evm_observers", None)
    if not isinstance(observers_by_chain, dict):
        raise HTTPException(status_code=503, detail="reference EVM observers are not configured")
    chain = request.path_params.get("chain")
    observers = observers_by_chain.get(chain)
    if not isinstance(observers, tuple):
        raise HTTPException(status_code=503, detail="reference EVM observers are not configured for chain")
    return observers


@router.post("/{chain}/observe")
async def observe_reference_dex(
    chain: str,
    payload: ReferenceDexOperatorPayload,
    session: Annotated[AsyncSession, Depends(get_session)],
    observers: Annotated[tuple[EvmReadOnlyRpc, ...], Depends(get_reference_observers)],
) -> dict:
    """Explicitly invoke one durable, read-only reference DEX observation."""
    if payload.chain != chain:
        raise HTTPException(status_code=400, detail="path chain and payload chain must match")
    try:
