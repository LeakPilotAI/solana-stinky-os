...
def _unknown(reason: str, *, limit: int) -> dict[str, Any]:
    return {"status": "UNKNOWN", "mint": None, "records": [], "missing": [reason], "bounded": {"limit": limit}, "evidence_only": True}


async def market_lifecycle_for_mint(session: AsyncSession, mint: str, *, limit: int = 100, as_of: datetime | str | None = None) -> dict[str, Any]:
    """Return measured lifecycle observations for one mint, bounded and as-of safe."""
    mint = str(mint or "").strip(); bounded_limit = max(1, min(int(limit), 500))
    if not mint: return _unknown("mint", limit=bounded_limit)
    cutoff = _parse_as_of(as_of)
    if as_of is not None and cutoff is None:
        result = _unknown("invalid_as_of", limit=bounded_limit); result["mint"] = mint; return result
    params: dict[str, Any] = {"mint": mint, "limit": bounded_limit}; cutoff_clause = ""
    if cutoff is not None:
        cutoff_clause = "AND observed_at <= :as_of AND ingested_at <= :as_of"; params["as_of"] = cutoff
    try:
        rows = (await session.execute(text(f"""
            ...
            WHERE mint = :mint {cutoff_clause}
            ORDER BY horizon_seconds ASC, observed_at ASC, id ASC
            LIMIT :limit
        """), params)).mappings().all()
    except Exception:
        result = _unknown("market_outcome_observations", limit=bounded_limit); result["mint"] = mint
        if cutoff is not None: result["as_of"] = cutoff.isoformat(); result["temporal_cutoff_enforced"] = True
        return result
    ...
    for row in rows:
        ...
        records.append(item)
    result: dict[str, Any] = {"status": "OBSERVED" if records else "UNKNOWN", "mint": mint, "records": records,
        "missing": [] if records else ["market_outcome_observations"], "bounded": {"limit": bounded_limit},
