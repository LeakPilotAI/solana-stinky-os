"""Describe measured market lifecycle paths without predicting outcomes."""
...
METRIC_KEYS = ("price_usd", "liquidity_usd", "volume_5m_usd", "volume_usd", "fdv_usd", "market_cap_usd")
...
def analyze_market_lifecycle(records: list[dict[str, Any]], *, limit: int = 100) -> dict[str, Any]:
    ...
    if not bounded_records:
        return {"status": "UNKNOWN", "observed_horizons": [], "observed_record_count": 0,
                "metrics": {}, "missing": ["market_outcome_observations"],
                "bounded": {"limit": bounded_limit}, "evidence_only": True}
    ...
    for key in METRIC_KEYS:
        ...
        if values:
            ...
            metrics_out[key] = {"first": {"horizon": first_horizon, "value": first_value},
    ...
    return {"status": "OBSERVED", "observed_horizons": horizons,
            "observed_record_count": len(bounded_records), "metrics": metrics_out,
            "missing": [], "bounded": {"limit": bounded_limit},
    ...
    if isinstance(metrics, dict):
        for key in METRIC_KEYS:
            ...
            if not isinstance(item, dict):
                continue
            first = item.get("first") if isinstance(item.get("first"), dict) else {}
