...
from stinky_core.evm_proxy_history import build_proxy_authority_change_history, derive_proxy_authority_changes
...
def test_history_records_implementation_and_admin_address_changes():
    ...
    assert rows[0].before_address == IMPL_A
    assert rows[0].after_address == IMPL_B
    assert rows[0].before_providers == PROVIDERS
    assert rows[0].after_providers == PROVIDERS
    assert all(row.status == "UNVERIFIED_PROXY_AUTHORITY_CHANGE_EVIDENCE" for row in rows)


def test_beacon_transition_preserves_both_beacon_and_resolved_implementation_history():
    before = snapshot(100, beacon=evidence(BEACON_A), beacon_impl=evidence(IMPL_A))
    after = snapshot(150, beacon=evidence(BEACON_B), beacon_impl=evidence(IMPL_B))
    rows = derive_proxy_authority_changes(before, after)
...
def test_evidence_appearance_and_disappearance_remain_descriptive_not_inferred():
    before = snapshot(100, eip_impl=None, eip_admin=evidence(ADMIN_A))
    after = snapshot(101, eip_impl=evidence(IMPL_A), eip_admin=None)
    rows = derive_proxy_authority_changes(before, after)
    assert [(row.field, row.classification) for row in rows] == [
...
def test_selector_evidence_is_also_temporally_preserved():
    before = snapshot(100, implementation=evidence(IMPL_A), admin=evidence(ADMIN_A))
    after = snapshot(120, implementation=evidence(IMPL_B), admin=evidence(ADMIN_B))
    rows = derive_proxy_authority_changes(before, after)
...
def test_history_builds_only_between_adjacent_observed_snapshots():
    ...
    assert [(row.from_block, row.to_block, row.before_address, row.after_address) for row in rows] == [
        (100, 150, IMPL_A, IMPL_B),
        (150, 200, IMPL_B, IMPL_A),
    ]
...
def test_history_rejects_cross_contract_and_non_monotonic_time():
    ...
    with pytest.raises(EvmRpcError, match="strictly increasing"):
        derive_proxy_authority_changes(snapshot(100), snapshot(100))
